...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	RECT wrc = { 0,0,window_w,window_h };
	//自動でサイズを補正する
	...
	result = device->CreateCommittedResource(&textureHeapProp,D3D12_HEAP_FLAG_NONE,&textureResourceDesc,D3D12_RESOURCE_STATE_GENERIC_READ,nullptr,IID_PPV_ARGS(&texBuff));
	...
	while (true) {
		...
		if (key[DIK_SPACE]) {
			FLOAT clearColor[] = { 0.1f,0.25f, 0.1f,0.0f };
			commandList->ClearRenderTargetView(rtvHandle, clearColor, 0, nullptr);
		}
		// 4 ----------------------描画コマンドここから--------------------------
		...
		scissorRect.right = scissorRect.left + window_w; // 切り抜き座標右
		scissorRect.top = 0; // 切り抜き座標上
		scissorRect.bottom = scissorRect.top + window_h; // 切り抜き座標下
